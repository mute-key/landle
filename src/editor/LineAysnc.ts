...
export namespace LineType {
     ...
     * need to perform multiple edits with single callback, this will be very
     * useful and ActiveEditor.#editSwitch need be rewriten other than switch.
     * 
     * - asdasd - asdasdasd
     * 
     */
    export const enum LineEditType {
        APPEND = 1 << 0, // asdfg wqw qweqwe
        PREPEND = 1 << 1,
        REPLACE = 1 << 2,
        CLEAR = 1 << 3,
        DELETE = 1 << 4
...
export abstract class Line {
     ...
     *itself and stops further iteration to prevent no more changes to be
     *applied to when the for loop is finished, it will be stacked into _line_edit_
 
     * this iteration could well have been done in array.reduce but it asdasd
     * does unnecessary exection in the iteartion. so thats why it is for loop
     ...
     * 
     * mismatch forces to return either a typed object or undefined because
     * 
    ...
    protected iterateNextLine = (
        ...
        nextLineBreakCallback?: (line: string) => boolean,
        trueConditionCallback?: (line: vscode.TextLine) => void
    ): LineType.IterateNextLineType => {
     ...
     * orarray of them to schedule the document edit. if the selection is
     * eitherof empty (whitespaces only) or a single line, the range should
     * bethe whole line.
