...
var commandId = {};
(() => {
  for (const c of package_default.contributes.commands) {
    if (c.command) {
...
var keybindings = package_default.contributes.keybindings;
  ...
  autoTriggerOnSaveEvent = () => {
    return vscode2.workspace.onWillSaveTextDocument((event) => {
      if (config.autoTriggerOnSave !== "disabled" && this.#directCall) {
        vscode2.commands.executeCommand(package_default.name + "." + config.autoTriggerOnSave, { includeEveryLine: true });
      }
    });
  };
  ...
  editInRange = async (lineCallback) => {
    try {
      if (!this.#documentSnapshot(this.#editor.document.getText()) && this.#editor) {
        const success = await this.#editor.edit(async (editBuilder) => {
...
var registerTextEditorCommand = (context, keys, command) => {
  return Object.keys(keys).map((key) => {
    if (key in command) {
      const commandString = [package_default.name, key].join(".");
      return vscode7.commands.registerTextEditorCommand(commandString, (editor, edit, params = defaultParam) => {
